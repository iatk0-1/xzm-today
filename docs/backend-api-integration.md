...
6. 管理端订单列表分页：`GET /admin/orders-manage/orders` 会忽略 `page`/`size` 并返回全量订单，现已标记为废弃；订单管理页已改用 `GET /admin/orders-manage/orders/page`，响应为 `{items,page,size,total}`，`page` 从 1 开始，`total` 因全局 `long` 序列化规则返回字符串，前端比较前需转数字。旧接口计划在下一版本删除，删除前需确认没有其他调用方。
